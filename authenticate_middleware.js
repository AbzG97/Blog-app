...
const authenticate_token = async (req, res, next) => {
    try {
        const retrieved_token = req.header("Authorization").replace("Bearer ", ""); // get the bearer token from req header Authorization done in postman it's generated by jwt
        const decoded_token = JWT.verify(retrieved_token, 'SecretToken'); // verify tokens using the secret / public key
